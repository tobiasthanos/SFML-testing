...
Editor::Ptr Editor::Create( const sf::String& text ) {
	Editor::Ptr ptr( new Editor );
...
const sf::String& Editor::GetText() const {
	return m_string;
}

void Editor::AppendText( const sf::String& text ) {
	m_string += text;
}

void Editor::PrependText( const sf::String& text ) {
	m_string = text + m_string;
}

void Editor::SetCursorPosition( std::size_t new_position ) {
...
void Editor::HideText( sf::Uint32 c ) {
...
sf::Uint32 Editor::GetHideCharacter() const {
	return m_text_placeholder;
}

std::size_t Editor::GetPositionFromMouseX( int mouse_pos_x ) {
...
void Editor::RecalculateVisibleString() const {
	float text_padding( Context::Get().GetEngine().GetProperty<float>( "Padding", shared_from_this() ) );
	const std::string& font_name( Context::Get().GetEngine().GetProperty<std::string>( "FontName", shared_from_this() ) );
	unsigned int font_size( Context::Get().GetEngine().GetProperty<unsigned int>( "FontSize", shared_from_this() ) );
	const sf::Font& font( *Context::Get().GetEngine().GetResourceManager().GetFont( font_name ) );
...
void Editor::MoveCursor( int delta ) {
...
void Editor::HandleTextEvent( sf::Uint32 character ) {
...
void Editor::HandleKeyEvent( sf::Keyboard::Key key, bool press ) {
	if( !press || !HasFocus() ) {
		return;
	}
...
void Editor::HandleMouseEnter( int /*x*/, int /*y*/ ) {
	if( !HasFocus() ) {
		SetState( PRELIGHT );
	}
}

void Editor::HandleMouseLeave( int /*x*/, int /*y*/ ) {
	if( !HasFocus() ) {
		SetState( NORMAL );
	}
}

void Editor::HandleMouseButtonEvent( sf::Mouse::Button button, bool press, int x, int /*y*/ ) {
...
void Editor::HandleUpdate( float seconds ) {
	if( !HasFocus() ) {
		return;
	}
...
void Editor::HandleFocusChange( const Widget::Ptr& focused_widget ) {
	if( HasFocus() ) {
		m_elapsed_time = 0.f;
		m_cursor_status = true;
	}
...
sf::Vector2f Editor::CalculateRequisition() {
	const std::string& font_name( Context::Get().GetEngine().GetProperty<std::string>( "FontName", shared_from_this() ) );
	unsigned int font_size( Context::Get().GetEngine().GetProperty<unsigned int>( "FontSize", shared_from_this() ) );
...
std::size_t Editor::GetVisibleOffset() const {
	return m_visible_offset;
}

std::size_t Editor::GetCursorPosition() const {
	return m_cursor_position;
}

const sf::String& Editor::GetVisibleText() const {
...
std::size_t Editor::GetMaximumLength() const {
	return m_max_length;
}

void Editor::SetMaximumLength( std::size_t max_length ) {
